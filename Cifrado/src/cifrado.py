...
def construye_diccionario_codificacion(k):
    ...
    for i in range(len(alfabeto)-1):
        if i+k > (len(alfabeto)-1):
            result[alfabeto[i]] = alfabeto[(i+k) - (len(alfabeto)-1)]
        else:
            result[alfabeto[i]] = alfabeto[i+k]
    return result 
...
def cifra_mensaje(texto, k):
    ...
    '''
    dict_codificado = construye_diccionario_codificacion(k)
    texto_codificado = ""
    for letra in texto:
        if letra not in dict_codificado:
            texto_codificado += letra
        else:
            texto_codificado += dict_codificado[letra]
...
def descifra_mensaje(texto, k):
    ...
    '''
    dict_descodificado = construye_diccionario_descodificacion(k)
    texto_descifrado = ""
    for letra in texto:
        if letra not in dict_descodificado:
            texto_descifrado += letra
        else:
            texto_descifrado += dict_descodificado[letra]
    return texto_descifrado
        

def construye_diccionario_descodificacion(k):
    result = dict()
    alfabeto = string.ascii_lowercase + string.ascii_uppercase
    for i in range(len(alfabeto)-1):
        result[alfabeto[i]] = alfabeto[i-k]
...
def es_palabra_valida(palabra, vocabulario):
    ...
    '''
    result = False
    if palabra.strip(" !@#$%^&*()-_+={}[]|\:;'<>?,./\"¡¿").lower() in vocabulario:
        result = True
    return result
# EJERCICIO 6:
...
def rompe_codigo_cifrado(texto, vocabulario):
    ...
    posibles_k = range(27)
    for k in posibles_k:
        aciertos[k] = cuenta_palabras_validas(descifra_mensaje(texto, k), vocabulario)
    mejor_desplazamiento = max(aciertos,key = aciertos.get)
