...
Registro = namedtuple('Registro', 'genero edad altura peso')
# EJERCICIO 1:
def lee_registros(fichero):
    ...
    with open(fichero,'r',encoding='UTF-8') as f:
        lineas = csv.reader(f)
        result = [Registro(linea[0],float(linea[1]),float(linea[2]),float(linea[3])) for linea in lineas]
        return result
...
def pesos_alturas_por_edades(registros, edad_min, edad_max):
    ...
    '''
    result = [(registro.peso,registro.altura) for registro in registros if edad_min <= registro.edad <= edad_max]
    return result
...
def pesos_alturas_por_genero(registros, genero):
    ...
    '''
    result = [(registro.peso,registro.altura) for registro in registros if registro.genero == genero]
    return result
...
def calcula_recta_regresion(puntos):
    ...
    de la recta, se seguirán los siguientes pasos:
      ...
      - Calcular los valores de a y b con las siguientes fórmulas:
            ...
            b = (sum_y - a*sum_x) / n
    '''
    sum_x = sum([x for x,_ in puntos])
    sum_y = sum([y for _,y in puntos])
